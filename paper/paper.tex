...
We adopt Working Hypothesis~F --- that the operational holonomy group
is finite --- as an explicit input. Under this hypothesis, Klein's
classification of finite $\SO(3)$ subgroups together with conditions
(E1)--(E5) selects $A_5$ as the minimal admissible group; $A_5$ thus
emerges from the discrete framework rather than being postulated.
...
\paragraph{Working Hypothesis~F (Finite Identifiability).}
Throughout this paper we adopt the following as an explicit working
input.

\begin{hypothesis}[Finite Identifiability, Hypothesis~F]
\label{hyp:F}
The operational holonomy group $\hop$ is finite:
\[
  \hop := \mathrm{Im}\!\bigl(\rho:\pi_1(\Gamma)\to\Sym(S)\bigr)
  \quad \text{is finite.}
\]
\end{hypothesis}

We explore the consequences of this finiteness hypothesis.
An information-theoretic and operationally independent proof of
Hypothesis~F is a future task (Open Problem~G14), but the operational
motivation is that any measurement procedure with finite observational
resolution can impose only a finite number of topological invariants on
a cycle.

\paragraph{Supporting evidence: Planck length and Planck time.}
Hypothesis~F is the \emph{input} of all theorems in this paper; the
following is positioned not as a proof but as independent supporting
evidence that the picture of infinite continuous resolution breaks
down physically (Section~\ref{sec:nogo}).
...
\begin{itemize}
  ...
  \item \textit{Minimum length thought experiment.}
    ...
    suggests that operationally verifying $\Delta x \lesssim \ell_{\mathrm{P}}$ is
    difficult or impossible~\cite{Maggiore1994}. This is consistent with
    the motivation for Hypothesis~F: finite observational resolution
    implies a finite number of distinguishable states.
  ...
  \item \textit{Entropy bound (finite degrees of freedom).}
    ...
    boundary rather than its volume~\cite{tHooft1993,Susskind1995,Bousso2002}, is
    linked to the finiteness motivation of Hypothesis~F.
\end{itemize}

The above evidence supports the physical intuition of the logical chain
(i) ``finite identifiability'' $\to$ (ii) ``smooth-local holonomy
interpretation fails'' $\to$ (iii) ``defect monodromy or discretization
(lattice),'' but it does not constitute a proof of Hypothesis~F.
...
\begin{description}
  ...
  \item[\tagP\ Physics (conditional).]
    Working Hypothesis~F restricts the implementation of finite holonomy
    to either ``defect monodromy'' or ``discretization (lattice)''
    (Section~\ref{sec:nogo}). Claims in this layer depend on Hypothesis~F and on
    bridge hypotheses (B2, B3, B5).
...
\begin{tikzpicture}[>=Stealth]
  ...
  \node[font=\normalsize\bfseries, text=cP, anchor=west]
    ...
    {Physics \textit{\small (Conditional --- depends on Hyp.\,F + B2, B3, B5)}};
  \node[figitem, anchor=north west]
    at ([xshift=12pt, yshift=-14pt] badgeP.south west)
    {$\bullet$\ \textbf{Hyp.\,F}: operational holonomy group $\hop$ is finite
       (Planck-scale motivation)\\[6pt]
  ...
  \draw[layerarrow] (layerM.south) -- (layerP.north)
    ...
    {Hyp.\,F\;+\;No-go\;$\Rightarrow$\;discrete framework};
...
Layer~\tagP\ requires Hypothesis~F and bridge hypotheses B2, B3, B5.
...
the no-go theorem leaves open under Working Hypothesis~F.  As a
...
(E5) Geometric realisation &
  Working Hypothesis~F $\to$ finite $\SO(3)$ subgroup $\to$
...
numbers. Within the discrete gauge structure imposed by Working
Hypothesis~F, $A_5$ is selected, and the coincidence of geometry and
representation theory at the level of the number field algebraically
constrains the cosmological scale (as an application example).
...
Under Working Hypothesis~F, any non-trivial implementation of finite
holonomy is restricted to one of:
\begin{enumerate}
...
\begin{tikzpicture}[
  ...
  \node[box] (start) {Finite holonomy\\under Hypothesis~F};
...
This section develops the discrete implementation that Working Hypothesis~F forces after the no-go
theorem rules out the smooth-local branch (Section~\ref{sec:nogo}).
...
the no-go theorem leaves open under Working Hypothesis~F, and shows
that the algebraic structure of $A_5$ constrains the scale of $\etaB$.
...
\begin{description}
  ...
  \item[G14 (Independent justification of Working Hypothesis~F).]
    Information-theoretic or operational proof that finite information
    capacity implies a finite operational holonomy group.  Clarification
    of the relationship with the continuous gauge groups $\mathrm{SU}(2)$
    and $\mathrm{SU}(3)$.
...
Under Working Hypothesis~F, this obstruction result leaves a restricted
finite discrete framework as the natural setting for further
construction. Within that setting, and restricting to finite subgroups
of $\SO(3)$ subject to conditions~(E1)--(E5), Klein's classification
selects $A_5$ as the minimal admissible group in the present algebraic
scheme. In this precise sense, $A_5$ is not postulated as an isolated
physical dogma, but emerges as the smallest candidate compatible with
the imposed structural requirements.
...
but the full dynamical origin of the bridge hypotheses, the microscopic
...
realizing the finite holonomy framework. In this sense, the paper should
be read as establishing a structural obstruction and a conditional
compression principle, rather than as claiming a completed microscopic
theory.
